...
同一頁裡多個元件讀同一支端點仍然只發一次 —— 那本來就該共用。

**只有一種情況該關掉**：掛在 layout 上、每頁都要、而且幾乎不會變的資料
（站台設定這一類）。那種資料的快取不會被清，開著等於每次換頁都多一個
阻塞的來回；這種請一併給固定的 `key`，並在那裡寫明理由。關之前先問：
**這份資料在後台被改過之後，使用者會不會在不重新整理的情況下看到舊的？**
會的話就不該關。

參考實作：`app/composables/api/useDemoApi.ts`、`app/composables/api/useAuth.ts`。
判斷邏輯與「試過但行不通的做法」見 `app/composables/useApiFetch.ts`，
